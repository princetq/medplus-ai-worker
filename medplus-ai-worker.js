...
async function handlePubMedSearch(body, env) {
  ...
  const sres = await fetch(searchUrl, { headers: { 'user-agent': 'MedPlusAIPro/1.5.2' } });
  ...
  const fres = await fetch(fetchUrl, { headers: { 'user-agent': 'MedPlusAIPro/1.5.2' } });
...
async function handlePmcFullText(body, env) {
  ...
  const ires = await fetch(idUrl, { headers: { 'user-agent': 'MedPlusAIPro/1.5.2' } });
  ...
  const res = await fetch(url, { headers: { 'user-agent': 'MedPlusAIPro/1.5.2' } });
